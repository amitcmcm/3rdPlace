...
class WalkNetwork(nn.Module):
    ...
    def forward(self, x, m, frac, flen, adjust = True):    
        ...
        # print(frac.shape)
        frac, lidx, ridx = frac[..., 0], frac[..., 1], frac[..., 2] #selecting the values from the last dimension of the frac tensor
...
class WalkModule(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # every 5th timestep or element is selected (downsampling). only the first three channels of y (probably the 3 classes)
        self.ys.append(y[:, ::SAMPLE, :3].cpu())
        self.ss.append(s[:, ::SAMPLE].cpu())
        # f and i are also stored in lists. Here, i (index tensor) is downsampled by dividing it by SAMPLE and converted to integers using .long().
        self.fs.append(f)
    ...
    def on_validation_epoch_end(self):
        fs = np.concatenate(self.fs) #the file names, concatenated in one array.
        idxs = torch.cat(self.idxs)
        uset = list(set(fs)) #uset stores unique values from fs, representing unique file identifiers
        ...
        for f in uset: #3 dictionaries are initialized and for each unique value in uset, the code initializes a key in each dict (pred_dict, ct_dict, and target_dict).
            l = 1000 * math.ceil( (idxs[fs == f].max() + slen) / 1000)
            pred_dict[f] = np.zeros((l, 3), dtype = np.float32)
            ct_dict[f] = np.zeros((l, 3), dtype = np.float32) # for aggregating and normalizing the predictions across overlapping sequences
            target_dict[f] = np.zeros((l, 3), dtype = np.float32)

        for i in range(len(ys)): #predictions, targets, and counts are added to pred_dict, target_dict, and ct_dict, respectively, indexed by fs[i]. Each array is sliced by idxs[i]:idxs[i] + slen to ensure values are placed correctly along the sequence length.
            pred_dict[fs[i]][idxs[i]:idxs[i] + slen] += yps[i] * s[i]
            target_dict[fs[i]][idxs[i]:idxs[i] + slen] = ys[i] * s[i]
            ct_dict[fs[i]][idxs[i]:idxs[i] + slen] += s[i] # If multiple batches have predictions overlapping at the same time steps, those counts accumulate, allowing for proper averaging later.
        ...
        final_ys, final_yps = [], []
        for k in ct_dict: # calculates final_ys and final_yps by appending slices of target_dict and pred_dict, divided by ct_dict to account for counts(?)
            f = ct_dict[k][:, 0] > 0
        ...
        if len(fs) < 1000: return  # If the length of fs is less than 1000, the function exits early. maybe for sanity check?


        aps = []
        labels = 'htw'
        for i in range(3): #probably iterating over the 3 classes, with final_ys being real labels and final_yps the predictions
        ...
        # s, ys, and yps are flattened and downsampled (taking every 20th element) for efficient precision scoring.
        ...
        aps = []
        labels = 'htw'
        for i in range(3): # Average precision scores for each channel are calculated again on this downsampled data and logged as a summary metric (val_aps).
